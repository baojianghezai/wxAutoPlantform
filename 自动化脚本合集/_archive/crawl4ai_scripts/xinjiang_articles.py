#!/usr/bin/env python3
"""信源文章爬虫：爬取各信源首页文章列表，提取标题、日期、链接，生成汇总页。"""
import os
import re
import json
import asyncio
...
# 信源配置：name, url, type, 以及针对该站点的文章链接提取规则
# pattern: 匹配文章链接的正��
# date_pattern: 从markdown中提取日期的正则（可选）
# title_selector: CSS选择器，用于从详情页提取标题（可选）
SOURCES = [
    # 政策法规信源
    {"name": "国务院", "url": "https://www.gov.cn", "type": "政策法规",
     "pattern": r"/zhengce/(?:content|jiedu)/\d{6}/[^)"'"'"'<>\s]+\.htm",
     "date_in_url": True},
    {"name": "最高人民法院", "url": "https://www.court.gov.cn", "type": "政策法规",
     "pattern": r"/(?:zixun|fabu)/xiangqing/\d+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "人力资源和社会保障部", "url": "https://www.mohrss.gov.cn", "type": "政策法规",
     "pattern": r"\.gov\.cn/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "国家税务总局", "url": "https://www.chinatax.gov.cn", "type": "政策法规",
     "pattern": r"\.gov\.cn/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "司法部", "url": "https://www.moj.gov.cn", "type": "政策法规",
     "pattern": r"\.gov\.cn/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "国家医疗保障局", "url": "https://www.nhsa.gov.cn", "type": "政策法规",
     "pattern": r"\.gov\.cn/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "国家统计局", "url": "https://www.stats.gov.cn", "type": "政策法规",
     "pattern": r"\.gov\.cn/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "教育部", "url": "https://www.moe.gov.cn", "type": "政策法规",
     "pattern": r"\.gov\.cn/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "全国工商联", "url": "https://www.acfic.org.cn", "type": "政策法规",
     "pattern": r"\.org\.cn/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "青岛市人社局", "url": "https://hrss.qingdao.gov.cn", "type": "政策法规",
     "pattern": r"\.gov\.cn/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "中国法律服务网", "url": "https://www.12348.gov.cn", "type": "政策法规",
     "pattern": r"\.gov\.cn/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "中国裁判文书网", "url": "https://wenshu.court.gov.cn", "type": "政策法规",
     "pattern": r"\.gov\.cn/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    # 科技学术信源
    {"name": "中国科学院", "url": "https://www.cas.cn", "type": "科技学术",
     "pattern": r"\.cas\.cn/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "清华大学经管学院", "url": "https://www.sem.tsinghua.edu.cn", "type": "科技学术",
     "pattern": r"\.edu\.cn/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "北京大学光华管理学院", "url": "https://www.gsm.pku.edu.cn", "type": "科技学术",
     "pattern": r"\.edu\.cn/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "中国人民大学劳人院", "url": "https://slhr.ruc.edu.cn", "type": "科技学术",
     "pattern": r"\.edu\.cn/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "国家自然科学基金委", "url": "https://www.nsfc.gov.cn", "type": "科技学术",
     "pattern": r"\.gov\.cn/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    # 企业行业信源
    {"name": "华为", "url": "https://www.huawei.com", "type": "企业行业",
     "pattern": r"\.com/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "腾讯", "url": "https://www.tencent.com", "type": "企业行业",
     "pattern": r"\.com/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "阿里巴巴", "url": "https://www.alibabagroup.com", "type": "企业行业",
     "pattern": r"\.com/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "字节跳动", "url": "https://www.bytedance.com", "type": "企业行业",
     "pattern": r"\.com/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "德勤中国", "url": "https://www.deloitte.com/cn", "type": "企业行业",
     "pattern": r"\.com/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "麦肯锡", "url": "https://www.mckinsey.com.cn", "type": "企业行业",
     "pattern": r"\.com\.cn/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "光辉国际", "url": "https://www.kornferry.com", "type": "企业行业",
     "pattern": r"\.com/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "美世", "url": "https://www.mercer.com.cn", "type": "企业行业",
     "pattern": r"\.com\.cn/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "虎嗅", "url": "https://www.huxiu.com", "type": "企业行业",
     "pattern": r"/article/\d+",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "智联招聘", "url": "https://www.zhaopin.com", "type": "企业行业",
     "pattern": r"\.com/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "BOSS直聘", "url": "https://www.zhipin.com", "type": "企业行业",
     "pattern": r"\.com/[^)"'"'"'<>\s]+\.html",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    # 自媒体���群信源
    {"name": "三茅人力资源网", "url": "https://www.hrloo.com", "type": "自媒体社群",
     "pattern": r"/article/\d+",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
    {"name": "KnowYourself", "url": "https://www.knowyourself.cc", "type": "自媒体社群",
     "pattern": r"/article/\d+",
     "date_in_md": r'(\d{4}-\d{2}-\d{2})'},
]
...
def extract_articles_from_markdown(md, source):
    """从markdown中提取文章链接列表。"""
    links = list(dict.fromkeys(re.findall(source.get("pattern", r'https?://[^)"'"'"'<>\s]+'), md)))
    # 清理链接
    cleaned = []
    for link in links:
    ...
    try:
        async with AsyncWebCrawler(config=browser_config) as crawler:
            ...
            if not result.success:
                ...
                return source["name"], source["type"], articles
            ...
            for i, link in enumerate(unique_links, 1):
                print(f"    详情 ({i}/{len(unique_links)})")
                detail = await crawl_article_detail(link)
...
async def main():
    ...
    for i, source in enumerate(SOURCES, 1):
        ...
        name, stype, articles = await crawl_source(source)
        summary.append({"name": name, "type": stype, "url": source["url"], "articles": articles})
